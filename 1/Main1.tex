...
\end{enumerate}
\item %28
Let $f:\mathbb{R} \to \mathbb{R}$ $g:\mathbb{R} \to \mathbb{R}$ be two non-differentiable functions. If $f'\brak{x}=(e^{\brak{f\brak{x}-g\brak{x}}})g'\brak{x}$ for all $x\in R$ ,and $f\brak{1}=g\brak{2}=1$, then which of the following statement \brak{s} is \brak{are} TRUE? 
\hfill  (JEE Adv.2018) \\ 
\begin{enumerate}
\begin{multicols}{2}
